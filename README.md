...
This package based on [Simple PHP SSO skeleton](https://github.com/zefy/php-simple-sso) package and [LaravelSSO](https://github.com/zefy/laravel-sso) package to made it suitable for Latest Laravel framework.
### Requirements
* Laravel 7.0+
* PHP 7.1+
...
```shell
$ composer require dhireshbudhiraja/laravelsso
```


Copy config file to Laravel project `config/` folder.
```shell
$ php artisan vendor:publish --provider="DhireshBudhiraja\LaravelSSO\SSOServiceProvider"
```


Create table where all brokers will be saved.
```shell
$ php artisan migrate --path=vendor/dhireshbudhiraja/laravelsso/database/migrations
```
...
Edit your `app/Http/Kernel.php` by adding `\DhireshBudhiraja\LaravelSSO\Middleware\SSOAutoLogin::class` middleware to `web` middleware group. It should look like this:
```php
protected $middlewareGroups = [
        'web' => [
            ...
            \DhireshBudhiraja\LaravelSSO\Middleware\SSOAutoLogin::class,
        ],
...
{
    $broker = new \DhireshBudhiraja\LaravelSSO\LaravelSSOBroker;
